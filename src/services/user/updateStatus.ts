...
function ServiceUpdateStatusUser(props?: ResponseHasResponseProps) {
	...
	const handleMutate = (params: PayloadUserUpdateStatus) => {
		mutate(params, {
			onSuccess: (data) => {
				Logger.debug('ServiceUpdateStatusUser execute handleMutate success', data);
			...
			onError: (error) => {
				Logger.error('ServiceUpdateStatusUser execute handleMutate error', error.toString());
				props?.onError?.();
			},
		});
	};
...
export default ServiceUpdateStatusUser;
