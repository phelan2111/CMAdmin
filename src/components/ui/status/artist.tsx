...
const parseUiStatusArtist: Record<EnumStatusArtist, ParseUiStatusArtist> = {
	[EnumStatusArtist.active]: {
		...
		bgClass: 'bg-green-300/20 px-2 py-1 rounded-3xl',
		text: 'ACTIVE',
	},
	[EnumStatusArtist.lock]: {
		className: 'bg-red-500',
		bgClass: 'bg-red-300/20 px-2 py-1 rounded-3xl',
		text: 'LOCK',
	},
	[EnumStatusArtist.inactive]: {
		className: 'bg-gray-500',
		bgClass: 'bg-gray-300/20 px-2 py-1 rounded-3xl',
		text: 'INACTIVE',
	},
};
function StatusArtist(props: StatusArtistProps) {
	...
	return (
		<div className={`flex items-center h-fit gap-2 ${statusItem.bgClass}`}>
			<div className={`w-2 h-2 ${statusItem.className} rounded-full`} />
			<p className='m-0 font-semibold h-fit text-base'>{Localize(statusItem.text)}</p>
		</div>
	);
}
